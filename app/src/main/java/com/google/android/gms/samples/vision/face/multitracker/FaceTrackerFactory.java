...
class FaceTrackerFactory implements MultiProcessor.Factory<Face> {
    ...
    public Recognition mRecognition;
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.drawOval(left, top, right, bottom, mBoxPaint);
            ...
            for (Landmark landmark : mFace.getLandmarks()) {
                int clx = (int) (landmark.getPosition().x * scaleX(1.0f));
                int cly = (int) (landmark.getPosition().y * scaleY(1.0f));
                canvas.drawCircle(clx, cly, 2, mBoxPaint);
